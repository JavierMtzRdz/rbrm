...
#' @importFrom magrittr %>%
#' @export
plot.cv_rbrm_simple <- function(.model, type.measure = c(
  ...
  tibble::as_tibble(result) %>%
    ...
    ggplot2::geom_vline(
      ggplot2::aes(
        xintercept = lambda_grid[best_lambda_idx], ,
        linetype = "Lambda min"
      ),
      colour = "#f94144"
    ) +
...
plot.cv_rbrm <- function(.model,
  ...
  data %>%
    ...
    {
      if (tolower(opt) == "gamma") {
        ggplot2::scale_x_continuous(limits = c(-0.1, 1.1))
      }
    } +
    ggplot2::scale_linetype_manual(values = c("dotted", "dashed")) +
    ggplot2::labs(
      x = opt_uppper, y = .model$type.measure,
      linetype = ggplot2::element_blank(),
      colour = ggplot2::element_blank()
    ) +
    ggplot2::theme_minimal() +
...
#' Prepare Data for Plotting cv_rbrm Object
#'
#' Extracts and summarizes cross-validation results for plotting.
#'
#' @param x A fitted object of class \code{cv_rbrm}.
...
#' @keywords internal
prepare_plot_data_cv_rbrm <- function(x, opt = "lambda", type.measure = NULL) {
  # --- Validate Inputs ---
  if (!inherits(x, "cv_rbrm")) {
    cli::cli_abort("Input object must be of class 'cv_rbrm'.")
  }
...
# --- Refactored plot.cv_rbrm ---

#' Plot Cross-Validation Curve from cv_rbrm Object
#'
...
#' @examples
#' \dontrun{
#' # Assuming 'cv_fit' is a result from cv_rbrm
#' plot(cv_fit)
...
#' }
plot.cv_rbrm <- function(x, type.measure = NULL, opt = "lambda", ...) {
  # --- 1. Prepare Data ---
  prep_results <- prepare_plot_data_cv_rbrm(x = x, opt = opt, type.measure = type.measure)
  ...
  if (opt_lower == "lambda") {
    ...
    limits_x <- c(
      min(0, min(plot_data$parameter, na.rm = TRUE) - 0.05),
